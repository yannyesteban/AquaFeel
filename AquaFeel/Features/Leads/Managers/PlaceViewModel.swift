...
class PlaceViewModel: ObservableObject {
    ...
    func doSearchPlaces(searchText: String, completion: @escaping (Result<[Place], Error>) -> Void) {
        guard let encodedSearchText = searchText.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) else {
            completion(.failure(NSError(domain: "SearchPlacesError", code: 1, userInfo: ["message": "Error encoding search text"])))
            return
        }
        ...
        guard let url = URL(string: urlString) else {
            completion(.failure(NSError(domain: "SearchPlacesError", code: 2, userInfo: ["message": "Error creating URL"])))
            return
        }
        ...
        URLSession.shared.dataTask(with: url) { data, _, error in
            ...
            guard let data = data else {
                completion(.failure(NSError(domain: "SearchPlacesError", code: 3, userInfo: ["message": "No data received"])))
                return
            }
            ...
            do {
                ...
                completion(.success(result.predictions))
            } catch {
                completion(.failure(NSError(domain: "SearchPlacesError", code: 4, userInfo: ["message": "Error decoding data: \(error.localizedDescription)"])))
            }
